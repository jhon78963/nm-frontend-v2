...
import { adaptStoreHeader } from './store-header.adapter';
...
export class StoreHeaderService {
  ...
  private readonly base = `${environment.apiUrl}/ecommerce/header`;

  getConfig(): Observable<StoreHeaderConfig> {
    return this.http
      .get<unknown>(this.base)
      .pipe(map(adaptStoreHeader));
  }

  saveConfig(payload: StoreHeaderPayload): Observable<StoreHeaderConfig> {
    return this.http.put<unknown>(`${this.base}/admin`, payload).pipe(
      map(adaptStoreHeader),
